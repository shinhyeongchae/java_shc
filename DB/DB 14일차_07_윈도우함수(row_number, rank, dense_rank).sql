/*
row_number() : 순서를 세서 1부터 차례대로 숫자를 할당, over() 안에 있는 것을 기준으로,
...
'2' > '12'
 */
-- row_number() 게시물 번호가 아닌 일반 번호로 바꿔주는 역할.
-- row_number() 순서를 세는데 over()  ()안에에 있는것을 기준으로 순서를 매긴다.
-- 일반 게시글을 등록 순서대로 정렬.
SELECT 
    row_number() over(order by bd_num)순서, bd_num, bd_title  
FROM
    community.board
WHERE
    bd_ca_name = '일반' ;
-- order by bd_num desc ; -- 여기서 order by bd_num desc;는 제일 최신에 올린 게시물을 바탕으로 순서를 매겨준다.
