...
CREATE TABLE `employee` (
	...
	`em_birth`	date	NULL,
	`em_address`	varchar(100)	NULL,
	`em_contact`	char(13)	NULL,
...
insert into employee(em_num, em_name, em_birth, em_address, em_contact, em_de_name, em_rn_name, em_year, em_enterd)
...
insert into pay (pa_em_num, pa_date, pa_salary, pa_name, pa_year)
	values(2000001,'2022-01-05',3880000, '차장', 1),
		  (2000002,'2022-01-05',4060000, '차장', 2),
          (2020001,'2022-01-05',2900000, '대리', 1);
-- 2022년 1월 5일에 등록된 모든 사원들의 월급을 지급했다.(1월 월급을 지급)
insert into pay(pa_em_num, pa_date, pa_salary, pa_name, pa_year)
	select em_num, '2022-01-05', rn_base + rn_add * em_year, em_rn_name, em_year from employee
		join `rank` on em_rn_name = rn_name
        where em_enterd < '2022-01-05';
