...
-- 문의사항은 회원이상 작성할 수 있고, 회원 이상만 읽을 수 있다
insert into category(ca_name, ca_r_authority, ca_w_authority, ca_secret_pos)
	values('공지사항',1, 10, 0), ('일반',0, 1, 0), ('문의사항',1, 1, 1);

-- abcd1234 회원이 일반 게시글에 제목 : 안녕하세요 내용 : 만나서 반갑습니다. 라는 글을 등록했습니다 
-- 최고관리자가 공지사항에 제목 : 공지사항1 내용 : 공지사항입니다
-- qwer1234회원이 문의 게시글에 제목 : 문의합니다 내용 : 비번을 바꾸고 싶어요 라는 글을 비밀글로 작성했다
...
insert into board(bd_ca_name, bd_title, bd_contents, bd_us_id, bd_secret, bd_ori_num) 
	select '일반', '안녕하세요1', '만나서 반갑습니다1', 'abcd1234', 0, max(bd_num) + 1
		from board;
...
insert into board(bd_ca_name, bd_title, bd_contents, bd_us_id, bd_secret, bd_ori_num, bd_depth)
    ...
    where bd_num = 3;
-- qwer1234회원이 5번 게시글에 답글을 남김. 제목 : 그래도 안되요. 내용 : 비번찾기를 이용해도 안됩니다
insert into 
    ...
    bd_ori_num, bd_depth + 1 from board
    where bd_num = 5;
-- 3번 게시글을 조회 했을 때(게시글 내용을 확인했을 때)필요한강좌 쿼리 => 조회수 증가
update board set views = views + 1 where bd_num = 3;
select * from board where bd_num = 3;
-- 추천/비추천을 했다 => 해당 게시글/댓글에 사용자가 처음 추천/비추하는 경우 => 등록, 아닌 경우는 => 수정
-- 조회수가 1증가

-- 1번 게시글에 qwer1234회원이 추천했을 때 쿼리문
insert into likes(li_us_id, li_state, li_target, li_target_num)
	values('qwer1234', 1, 'board', 1);
-- qwer1234 회원이 1번 게시글에 추천을 한번더 눌러 추천을 취소했을 때 쿼리문을 작성
...
-- 게시글을 등록 => insert
insert into board(bd_ca_name, bd_title, bd_contents, bd_us_id, bd_secret, bd_ori_num) 
	select '일반', '제목입니다2', '내용입니다 반갑습니다1', 'abcd1234', 0, ifnull(max(bd_num),0) + 1
		from board;
-- 첨부파일들을 등록 => insert
insert file(fi_name, fi_ori_name, fi_bd_num)
	values('uuid_a.txt',' uuid_a.txt',7),('b.txt', 'uuid_b.txt',7);
