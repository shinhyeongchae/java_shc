-- 일반 게시글을 조회 하는데, 게시글 번호, 게시글 제목, 댓글수 조회
select bd_num as 게시글번호, bd_title as 게시글제목, count(co_bd_num) as 댓글수  from board 
...
-- 모든게시글에 대해 게시글번호, 게시글 제목과 추천수와 비추천수를 조회
-- 1 제목1	1	0
-- count와 case를 같이 사용 
-- li_state가 1일때 개수를 세는 방법
-- 개수를 세는데 li_state가 1일때 세는 방법
select bd_num as 번호, bd_title as 제목, 
	count(case when li_state = 1 then li_state end) as 추천수,
    count(case when li_state = -1 then li_state end) as 비추천수
    from board 
...
select bd_num as 번호, bd_title as 제목, 
	ifnull(sum(li_state = 1),0) as 추천수,
	ifnull(sum(li_state = -1),0) as 비추천수
    from board 
	left join (select * from likes where li_target = 'board') as li
		on li_target_num = bd_num
    where bd_ca_name = '일반'
