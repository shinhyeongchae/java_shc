...
create table if not exists 학생(
	학생번호 int auto_increment, 
			-- auto_increment 는 1,2,3 을 입력하지않아도 자동증가시켜주는 메소드
    이름 char(6),
	primary key(학생번호)
);
-- 학생 테이블에 학과 속성을 추가
...
-- 기본키 삭제
alter table 학생 drop primary key; -- 에러 발생 auto_increment 때문에 
-- 기본키 추가
...
-- 속성명을 입력 => 테이블 생성 시 추가한 속성들의 순서가 중요하지 않음, 입력한 순서가 중요함
insert into 학부(학부명, 학부위치) values('기계 공학부', '그린대학교 1관 201호'); 
-- 학부 테이블에 전자공학부를 추가
-- 생략한 속성이 있는 경우, 생략한 속성에는 기본값으로 저장
-- 기본값을 설정 안 한 경우 NULL이 들어감 .
...
-- 컴퓨터공학부 학부위치가 그린대학교 2관 201호로 이동을 함, 이때 알맞은 커리문을 작성해야함
update 학부
	set
	  학부위치 = '그린대학교 2관 101호'
	where 
