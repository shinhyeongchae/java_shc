...
class Array<T/* extends Number*//*이코드를 추가하면 Number클래스 또는 상속받은 클래스들만 T에 올 수 있다.*/>{
	private T arr[];
	private int size = 0; //배열의 최대크기
	...
	/*?(와일드 카드) 제네릭 클래스에서 메소드에 제네릭 클래스의 객체가 오는데, 제네릭 타입이 서로 다른경우 
	 * 	메소드에서 호출할수 있게 해줌. 위예제에서 Integer 타입으로 된 array가 String 타입으로
	 *  된 arry2를 매개변수로 가져와서 print를 실행 = >와일드카드때문에 가능*/
