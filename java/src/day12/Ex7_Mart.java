...
public class Ex7_Mart {
			...
			case 1:
				...
				case 1 : case 2 :
					...
					listCount++;
						...
						//잘못된 서브메뉴를 선택하면
				default : 
					System.out.println("선택할수 없는 종류입니다.");
			    }
						break;
				//선택하면 메뉴가 2이면(제품입고)		
				case 2:
	...
	/*기능 : 제품등록에 대한 서브 메뉴를 출력하고, Scanner를 통해 서브 메뉴를 입력받아 서브메뉴를 얄려주는 메소드
	 * 매개변수 : Scanner => Scanner scan
	 * 리턴타입 : 입력받은 메뉴 = >정수 => int
	 * 메소드명 : selectsubMenu */
	 ...
	 * 														제품을 알려주는 메소드.
	 * 매개변수 : 선택한 subMenu ,Scanner =>Scanner scan
	 * 리턴타입 : 입력받은 메뉴 => 생성한 객체 =>Product
	 * 메소드명 : creatProduct*/
	public static Product createProduct(int subMenu, Scanner scan) {
		//등록하기 위한 공통된 제품정보를 입력
	...
	public static void printProductlist(Product productList[],int listCount){
		if(productList == null || listCount == 0){
			System.out.println("등록된 제품이없습니다");
			return;
		}
	...
	/*기능 : Scanner를 이용하여 제품 목록에서 제품과 수량을 입략하여 수량을 추가하는 메소드
	 * 매개변수 : Scanner, 제품 목록=>Scanner scan
	 * 리턴타입 : 입고 여부=> boolean
	 * 메소드명 :	addAmountProduct
	 * */
	public static boolean addAmountProductList(Scanner scan,Product list[],int listCount) {
		...
		//선택된 제품에 입고된 수량을 추가
		list[num-1].setAmount(amount);
		//입고가 완료됬다고 알려줌
		return true;
	}
	/*	기 능 : 스캐너를 이용하여 제품과 수량을 선택하면 해당 제품리스트에서 해당 제품을 수량에 맞게 꺼내서 제품을 돌려주는 메소드
	 *매개변수 : Scanner,  제품리스트(마트가 관리하는) => Scanner scan, Product list[], int listCount.
	...
	public static Product selectProduct(Scanner scan, Product list[],int listCount) {
		...
		Product selectbuyProduct = null;
		//선택된 제품을 복사해서 가져옴
		if(buyProduct instanceof SnackBox) {
			selectbuyProduct = new SnackBox((SnackBox)buyProduct);
		}else if(buyProduct instanceof Drink) {
			selectbuyProduct = new Drink((Drink)buyProduct);
		}else {
			return null;
		}
		//제고량보다 많은 수량을 입력한 경우
		...
		selectbuyProduct.setAmount(amount);
		//판매제품 리스트에서 선택한 수량을빼줌
		buyProduct.sumAmount(-amount);
		//선택한 제품을 알려줌
		return selectbuyProduct;
		
	}
	/*기능 : 제품 리스트가 주어지면 해당 제품 리스트의 합계를 구하여 알려주는 메소드
	 * 매개변수 : 제품리스트 => Product list[],intCount
	 * 리턴타입 : 합계 =>int
	 * 메소드명 : sumProductList*/
	...
	public static void returnProductList(Product list[], int ListCount, Product basket[], int basketCount) {
		...
		for(int i = 0; i<ListCount; i++) {
			for(int j=0; j<basketCount;j++) {
				...
				if(pi.getName().equals(pj.getAmount())) {
