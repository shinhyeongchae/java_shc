...
public class BaseballGame implements ConsoleProgram{
	...
	public void excute(int menu) {
		...
		case 1:
			...
			int count = 0; //while문 밖에서 카운트를 선언해야하는이뉴는 while문에서는 게임이 실행하기때문에
			while(true) {
				//숫자를 3개 입력
				inputNumbers(3);
				//횟수 1증가
	...
	public void inputNumbers(int count) {
		String str = "중복되지 않게 {0}~{1}사이의 {2}개 숫자 입력하세요 : ";
		...
		while(user.size() < count) {
			...
			//범위가 아닌 숫자를 입력했을 때, 중복될 때
				...
				System.out.println("중복이 되거나 범위를 벗어났습니다,. 다시 입력하세요");
				if(scan.hasNextInt()) {
					scan.nextLine();
				}
					System.out.println(formatStr);
			...
			//숫자가 아닌 문자를 입력했을 때
				...
				//숫자가 아닌 문자로 입력했을때, 범위가 아닌 숫자를 입력했을 때, 중복 될때
